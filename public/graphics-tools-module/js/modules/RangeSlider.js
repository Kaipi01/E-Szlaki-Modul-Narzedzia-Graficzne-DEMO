export default class RangeSlider extends HTMLElement {
  ...
  REFLECTED_ATTRIBUTES = ['min', 'max', 'step', 'value', 'disabled', 'value-precision'];
  ...
  static observedAttributes = this.REFLECTED_ATTRIBUTES;
  ...
  set min(min) {
    ...
    for (const thumb of this.#thumbs) {
      this.setAriaAttribute(thumb, 'min', min);
    }
  }
  set max(max) {
    this.setAttribute('max', max);
    for (const thumb of this.#thumbs) {
      this.setAriaAttribute(thumb, 'max', max);
    }
  }
  set step(step) {
  ...
  setAriaAttribute(element, name, value) {
    const attributeName = this.ARIA_ATTRIBUTES[name];
  ...
  connectedCallback() {
    ...
    this.#thumbs.forEach((thumb, index) => {
      ...
      this.setAriaAttribute(thumb, 'min', this.min);
      this.setAriaAttribute(thumb, 'max', this.max);
      if (!this.disabled) {
        thumb.setAttribute('tabindex', 0);
      }
    });
  ...
  #keyboardHandler = (event) => {
    const actionKeys = Object.keys(this.KEY_CODE_ACTIONS);
    const action = actionKeys.find((type) => this.KEY_CODE_ACTIONS[type].includes(event.code) && type);
  ...
  #updateValue(index, value, dispatchEvents = []) {
    ...
    const valuePrecision = Number(this.valuePrecision) || this.getPrescision(this.step) || 0;
  ...
  #updateThumb(index, value) {
    ...
    this.setAriaAttribute(this.#thumbs[index], 'value', value);
  }
...
//     connectedCallback() {
//         this.minValue = this.getAttribute("data-min") ?? 0;
//         this.maxValue = this.getAttribute("data-max") ?? 100;
//         this.currentValue = this.getAttribute("data-current") ?? this.maxValue;
//         this.render();
//         this.init()
//     }
...
//         this.rangeValueEl = this.querySelector('[data-value]')
//         this.rangeInput.addEventListener("input", (e) => this.rangeValueEl.textContent = e.target.value)
//     }

//     render() {  
//         this.innerHTML = `
//             <div class="range-slider">
//                 <input data-range-input type="range" min="${this.minValue}" max="${this.maxValue}" value="${this.currentValue}">
//                 <span data-value="${this.currentValue}">${this.currentValue}</span>
//             </div>
//         `
//     }
// }