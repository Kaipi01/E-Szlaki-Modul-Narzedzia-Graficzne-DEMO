...
class GTMCompressorSSEController extends AbstractController
{
    ...
    private const MAX_CONNECTION_TIMEOUT = 300; // 5 minut
    private const POLL_INTERVAL = 1; // 1 sekunda 
    ...
    /**
     * Nawiązuje połączenie SSE, które monitoruje postęp w czasie rzeczywistym
     * Wykorzystuje ReactPHP do efektywnego śledzenia zmian bez blokowania głównego wątku
     */
    ...
    public function trackCompressionProgress(string $processHash): Response
    {
        ...
        $this->logger->debug('Rozpoczęcie połączenia SSE z ReactPHP', ['processHash' => $processHash]);

        try {
            return new StreamedResponse(function () use ($processHash) {
                // Konfiguracja nagłówków SSE
                $this->configureSSEHeaders();

                // Ostatnio wysłany postęp
                $lastProgress = null;
                $lastStatus = null;

                // Ustawienie timeout dla połączenia
                $timeoutTimerId = Loop::addTimer(self::MAX_CONNECTION_TIMEOUT, function () use ($processHash, &$timeoutTimerId) {
                    $this->logger->warning('Timeout połączenia SSE', ['processHash' => $processHash]);
                    $this->sendSSEMessage('timeout', 0);
                    Loop::cancelTimer($timeoutTimerId);
                });

                // Ustawienie timera sprawdzającego postęp regularnie
                $progressTimerId = Loop::addPeriodicTimer(self::POLL_INTERVAL, function () use ($processHash, &$lastProgress, &$lastStatus, &$progressTimerId, &$timeoutTimerId) {

                    // Sprawdzenie czy klient jest nadal połączony
                    if (connection_aborted()) {
                        $this->logger->info('Klient rozłączony', ['processHash' => $processHash]);
                        Loop::cancelTimer($progressTimerId);
                        return;
                    }

                    // Pobranie aktualnego postępu
                    $progressData = $this->compressionTracker->getProgress($processHash);
                    $this->compressionTracker->showProgressLog($processHash);

                    // Jeśli brak danych, kontynuuj
                    ...
                    // Wysyłanie aktualizacji tylko gdy się zmieniły
                    if ($progressValue !== $lastProgress) {
                        $this->sendSSEMessage($progressStatus, $progressValue);
                    ...
                    // Jeśli proces zakończony, zatrzymaj pętlę
                    if ($progressStatus === ImageOperationStatus::COMPLETED || $progressStatus === ImageOperationStatus::FAILED) {
                        Loop::cancelTimer($progressTimerId);
                        Loop::cancelTimer($timeoutTimerId);
                    }
                });
                ...
                // KLUCZOWA ZMIANA: Nie używamy Loop::run(), które blokuje wątek
                // Zamiast tego, wykonujemy jedną iterację pętli i natychmiast zwracamy sterowanie

                // Wysyłamy inicjalne potwierdzenie, że połączenie SSE jest gotowe
                //$this->sendSSEMessage('connected', 0);

                // KLUCZOWA ZMIANA: Zamiast Loop::run() lub Loop::tick(), używamy własnej nieblokującej pętli
                // Utrzymujemy połączenie otwarte, ale NIE blokujemy głównego wątku
                // $startTime = time();

                // // Używamy pętli while z krótkim opóźnieniem, aby umożliwić przetwarzanie innych żądań
                // while (!connection_aborted() && (time() - $startTime < self::MAX_CONNECTION_TIMEOUT)) {
                //     // Krótka przerwa, aby nie obciążać CPU i pozwolić na przetwarzanie innych żądań
                //     usleep(100000); // 100ms

                //     // Opróżniamy bufor wyjścia po każdej iteracji
                //     $this->flushOutput();

                //     // Sprawdzamy, czy proces został zakończony
                //     $progressData = $this->compressionTracker->getProgress($processHash);
                //     if ($progressData !== null) {
                //         $progressStatus = $progressData['status'];
                //         if ($progressStatus === ImageOperationStatus::COMPLETED || $progressStatus === ImageOperationStatus::FAILED) {
    ...
    private function sendSSEMessage(string $status, int $progress): void
    {
        $eventName = $status === ImageOperationStatus::COMPLETED ? 'completed' : ($status === ImageOperationStatus::FAILED ? 'error' : 'progress');

        echo "event: $eventName\n";
        echo "data: $progress\n\n";
        echo ":" . str_repeat(" ", 2048) . "\n"; // Zapobieganie problemom z buforowaniem
    ...
    private function createErrorResponse(string $errorMessage): Response
    {
        ...
        $this->logger->error('Błąd podczas obsługi SSE', ['error' => $errorMessage]);
